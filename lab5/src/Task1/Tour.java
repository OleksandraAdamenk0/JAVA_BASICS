...
public class Tour implements Comparable<Tour>, Serializable {
    ...
    public static Tour fromString(String tour) {
        if (tour == null || tour.isEmpty()) throw new IllegalArgumentException("Empty string");

        String[] parts = tour.split(";");
        if (parts.length != 3) throw new IllegalArgumentException("Wrong format: " + tour);

        String city = parts[0].trim();
        String date = parts[1].trim();
        int concerts;

        try { concerts = Integer.parseInt(parts[2].trim()); }
        catch (NumberFormatException e) { throw new IllegalArgumentException("Concerts should be a number: " + parts[2]); }

        return new Tour(city, date, concerts);
    }

    @Override
    public String toString() {
        return city + ";" + date + ";" + concerts;
    }
}
